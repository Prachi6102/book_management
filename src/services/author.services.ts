...
export class AuthorServices {
    ...
    async addAuthor(name: string, biography: string, nationality: string): Promise<IAuthor | string> {
        ...
        if (existingAuthor) {
            ...
            const author: IAuthor = await queries.createAuthor(name, biography, nationality);
            return author;
        }
    }

    async updateAuthor(id: string, name: string, biography: string, nationality: string): Promise<IAuthor | string> {
        ...
        if (!existingAuthor) {
            ...
            if (name !== existingAuthor.name) {
                ...
                const existingNameAuthor: IAuthor | null = await queries.findByName(name);
                if (existingNameAuthor) {
                    return "Authorname already exists!";
                }
                updatedName = name;
            }
            const updatedAuthor: IAuthor | null = await queries.updateAuthor(id, updatedName, biography, nationality);
            if (!updatedAuthor) {
