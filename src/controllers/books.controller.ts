  ...
  try {
    ...
    if (categoryFilter && mongoose.Types.ObjectId.isValid(categoryFilter)) {
      if (typeof books === 'string') {
        ...
        books = books.filter(
          (book) => book.category.toString() === categoryFilter
        )
      }
    }
    ...
    if (authorFilter && mongoose.Types.ObjectId.isValid(authorFilter)) {
      if (typeof books === 'string') {
        ...
        books = books.filter((book) => book.author.toString() === authorFilter)
      }
    }
...
const addBook = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const Book: IBook | string = await service.addBook(
      title,
      author,
      category,
      ISBN,
...
const updateBook = async (req: Request, res: Response): Promise<void> => {
  try {
    const { id } = req.params
    const { title, author, category, ISBN, description, price } = req.body
    const Book: IBook | string = await service.updateBook(
      id,
      title,
      author,
      category,
